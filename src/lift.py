...
ent_terr = collections.defaultdict(collections.Counter)
...
for m in maps:
    ...
    for o in m["objects"]:
        ...
        try:
            ...
            continue
        ent_terr[entity(o)][t] += 1
        ent_count[entity(o)] += 1
...
ENT = sorted(ent_count)
EI = {e: i for i, e in enumerate(ENT)}
nE = len(ENT)
TERR = sorted(terr_total)
TIx = {t: i for i, t in enumerate(TERR)}
nT = len(TERR)
...
for e in ENT:
    for t in allowed[e]:
        ALLOWED[EI[e], TIx[t]] = 1.0

# ---- pass 2: observed + terrain-controlled expected via integral images ----
observed = np.zeros((nE, nE, len(BANDS)))
expected = np.zeros((nE, nE, len(BANDS)))


def integral(mat):
    return np.pad(np.cumsum(np.cumsum(mat, 0), 1), ((1, 0), (1, 0)))
        ...
        for e, pos in here.items():
            ...
            for cx, cy in cs:
                for bi, (lo, hi) in enumerate(BANDS):
                    obs = box(EII, cx - hi, cy - hi, cx + hi, cy + hi, W, H) - box(
                        EII,
                        cx - (lo - 1),
                        cy - (lo - 1),
                        cx + (lo - 1),
                    ...
                    tcnt = box(TII, cx - hi, cy - hi, cx + hi, cy + hi, W, H) - box(
                        TII,
                        cx - (lo - 1),
                        cy - (lo - 1),
                        cx + (lo - 1),
...
out = {
    ...
    "lift": {},
    "observed": {},
    "note": "lift>1 attract, <1 repel; nan/low-obs = sparse",
}
for a in ENT:
    out["lift"][a] = {
        b: [
            round(float(lift[EI[a], EI[b], k]), 3) if observed[EI[a], EI[b], k] >= MIN_OBS else None
...
def L(a, b, k):
    o = observed[EI[a], EI[b], k]
    return round(float(lift[EI[a], EI[b], k]), 2) if o >= MIN_OBS else None
...
for a, b in [
    ...
    ("GUARD", "REWARD_PICKUP"),
    ("TREASURE_CHEST", "GUARD"),
    ("MINE", "MINE"),
    ("TOWN", "TOWN"),
    ("RANDOM_RESOURCE", "RANDOM_RESOURCE"),
    ("MONOLITH_TWO_WAY", "TOWN"),
]:
    if a in EI and b in EI:
        print(f"  {a:<16}~ {b:<16} {[L(a, b, k) for k in range(3)]}")
# top attract / repel at short range (band0), confident only
print("\n=== strongest SHORT-RANGE (0-2) attractions ===")
pairs = [
    (a, b, lift[EI[a], EI[b], 0]) for a in ENT for b in ENT if observed[EI[a], EI[b], 0] >= MIN_OBS
]
for a, b, v in sorted(pairs, key=lambda r: -r[2])[:12]:
    print(f"  {a:<16}~ {b:<16} {v:.1f}x")
print("=== strongest SHORT-RANGE (0-2) repulsions ===")
for a, b, v in sorted(pairs, key=lambda r: r[2])[:12]:
    print(f"  {a:<16}~ {b:<16} {v:.2f}x")
print("\nwrote out/lift.json")
