...
import sys, os, json, glob, random, collections, argparse, hashlib
...
def _shuffled_objdist(fm, sig_real):
    ...
    Uses a deterministic seed derived from the object-type sequence so that
    re-running benchmark.py with the same --maps/--seeds args produces the same
    control values.
    """
    free = [
    ...
    # hashlib for a stable seed: Python's hash() is PYTHONHASHSEED-randomized per process
...
def coverage(n, seeds):
    ...
    for p in _corpus(n):
        try:
            ...
            print("  fit FAIL", os.path.basename(p), repr(e)[:50])
            continue
        ctrl = _shuffled_objdist(best[5], sig)
        rows.append(
            {
                "map": os.path.basename(p).replace(".h3m", ""),
                "objdist": round(best[2], 2),
                "total": round(best[0], 1),
    ...
    return {
        ...
        "mean_control_objdist": round(
            sum(r["control_objdist"] for r in rows) / max(1, len(rows)), 2
        ),
        "rows": rows,
    }
...
def reconstruct(target, seeds):
    ...
    return {
        "target": target,
        "best_total": round(best[0], 1),
        "best_objdist": round(best[2], 2),
        "control_objdist": round(ctrl, 2),
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    out = {
        "coverage": coverage(a.maps, a.seeds),
    ...
    print("\n=== BENCHMARK ===")
    print(
        f"COVERAGE   : mean obj-dist {c['mean_objdist']} ({c['pct_within_bar']}% <= {OBJDIST_BAR}) "
        f"over {c['n']} maps   [control {c['mean_control_objdist']}]"
    )
    print(
        f"VALIDITY   : pass-rate {v['pass_rate']} over {v['seeds']} seeds (reachable + density-in-tolerance)"
    )
    print(
        f"RECONSTRUCT: '{r.get('target')}' best obj-dist {r.get('best_objdist')} total {r.get('best_total')} "
