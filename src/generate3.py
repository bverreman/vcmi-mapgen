...
import vmaplib, vmapwrite, render, markov_terrain as MK
        ...
        for y in range(1, H - 1):
            for x in range(1, W - 1):
                ...
                if sum(base.values()) < thresh:
                    ...
                    base.update(M4["horiz"][(l, r)])
                    base.update(M4["vert"][(u, d)])
                sub = collections.Counter(
                    {c: base.get(c, 0) + 1 for c in choices}
                )  # restrict to local terrains
...
def place_towns(g, K, rnd, minsep):
    ...
    for y in range(2, H - 2):
        for x in range(2, W - 2):
            if g[y][x] in (2, 0, 5) and all(
                g[y + dy][x + dx] != WATER for dy in (-1, 0, 1) for dx in (-1, 0, 1)
            ):
                cand.append((x, y))
    rnd.shuffle(cand)
    towns = []
...
def astar(g, s, t):
    ...
    def cost(x, y):
        return {WATER: 40, LAVA: 12, 5: 3, 9: 30}.get(g[y][x], 1)
...
def scatter_clustered(g, level, occ, counter, rnd):
    ...
    for y in range(H):
        for x in range(W):
            t = g[y][x]
            pool = PAL["decor_by_terr"].get(str(t)) or []  # affinity-filtered by terrain
            if not pool:
                continue
            clump = field[y][x] ** 2.2 * 2.6  # peaks dense, troughs empty, ~avg 1
            dens = HD.get(t, 0) / 1000.0 * clump
            if (x, y) in occ or rnd.random() >= dens:
                continue
            tpl = rnd.choice(pool)
...
if __name__ == "__main__":
    ...
    for i, (x, y) in enumerate(towns):
        ...
        to["y"] = y
        to["l"] = 0
        to["instanceName"] = f"town_{i}"
        town_objs.append(to)
        for cc in mask_blocked(to.get("template", {}), x, y):
            occ.add(cc)
    counter = [0]
    decor = scatter_clustered(g, 0, occ, counter, rnd)
    ...
    render.render_map(norm, f"{ROOT}/out/render/gen3_map.png")
