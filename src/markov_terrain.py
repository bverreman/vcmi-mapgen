...
sys.path.insert(0,"src"); import render
...
def learn(level_index):
    ...
    for f in glob.glob(f"{ROOT}/out/maps/*.json"):
        m=json.load(open(f))
        if level_index>=len(m["terrain"]): continue
        g=m["terrain"][level_index]; H=len(g); W=len(g[0])
...
def _sample(counter, rnd):
    tot=sum(counter.values()); r=rnd.random()*tot; acc=0
...
def generate(model, W, H, rnd, thresh=12):
    ...
    for y in range(H):
        for x in range(W):
            ...
            dist=None
            if l is not None and u is not None and ul is not None and sum(model["full"][(l,u,ul)].values())>=thresh:
...
def learn4(level_index):
    ...
    pair=collections.defaultdict(collections.Counter)  # frozenset-ish (sorted 2 of nbrs) backoff via opposite pairs
    ...
    for f in glob.glob(f"{ROOT}/out/maps/*.json"):
        ...
        for y in range(1,H-1):
            for x in range(1,W-1):
                c=T[y][x]; l=T[y][x-1]; u=T[y-1][x]; r=T[y][x+1]; d=T[y+1][x]
                full[(l,u,r,d)][c]+=1; horiz[(l,r)][c]+=1; vert[(u,d)][c]+=1
...
def gibbs(grid, M4, marg, rnd, sweeps=5, thresh=10):
    ...
    for s in range(sweeps):
        for y in range(1,H-1):
            for x in range(1,W-1):
                l=grid[y][x-1]; u=grid[y-1][x]; r=grid[y][x+1]; d=grid[y+1][x]
                if sum(M4["full"][(l,u,r,d)].values())>=thresh:
                    dist=M4["full"][(l,u,r,d)]
                else:
                    dist=collections.Counter()
                    dist.update(M4["horiz"][(l,r)]); dist.update(M4["vert"][(u,d)])
                    if not dist: dist=marg
...
if __name__=="__main__":
    ...
    M=learn(0)
    M4=learn4(0)
    print(f"  contexts: full={len(M['full'])} pair={len(M['pair'])}  marginal terrains={dict(M['marg'].most_common())}")
    W=H=72
    gen=generate(M,W,H,rnd)                       # raster init
    def asmap(g,name): return {"name":name,"width":W,"height":H,
            "terrain":[[[{"t":t,"river":False,"road":False} for t in row] for row in g]],"objects":[]}
    ...
    for f in glob.glob(f"{ROOT}/out/maps/*.json"):
        ...
        if m["width"]<=margin if False else (60<=m["width"]<=90 and not m["twoLevel"]):
            real=m; break
    if real:
        real_terronly=dict(real); real_terronly["objects"]=[]
