...
import ontology as ON, vmaplib, render
...
print(
    f"Human maps: {len(human)}   RMG maps: {len(rmg)}   (density = objects per 1000 tiles, avg per map)\n"
)
...
for p in PURP:
    ...
    flag = (
        "  <-- RMG overuses"
        if ratio > 1.6 and r > 0.05
        else ("  <-- RMG underuses" if ratio < 0.6 and h > 0.05 else "")
    )
