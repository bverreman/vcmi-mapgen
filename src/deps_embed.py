...
def organic_layout(tree, rnd):
    ...
    pos[0] = (clamp(W / 2 + rnd.uniform(-0.12, 0.12) * W, W),
              clamp(H / 2 + rnd.uniform(-0.12, 0.12) * H, H))
    placed.append(pos[0]); heading[0] = rnd.uniform(0, 2 * math.pi)
    ...
    while queue:
        ...
        for i, c in enumerate(ch):
            if p == 0:                                  # root children fan out in any direction
                base = rnd.uniform(0, 2 * math.pi) if len(ch) == 1 else \
                    (2 * math.pi * i / len(ch) + rnd.uniform(-0.7, 0.7))
            ...
            if spot is None:                            # accept a closer spot rather than fail
                ...
                spot = (x, y, base)
            pos[c] = (spot[0], spot[1]); placed.append(pos[c]); heading[c] = spot[2]
            queue.append(c)
    return pos
...
def grow_zones(tree, pos, rnd):
    ...
    for nid in sorted(pos):
        ...
        zone[y][x] = nid; seed_tile[nid] = (x, y)
    ...
    while q:
        ...
        for dx, dy in NB4:
            nx, ny = x + dx, y + dy
            if 0 <= nx < W and 0 <= ny < H and zone[ny][nx] == -1:
                zone[ny][nx] = zone[y][x]; q.append((nx, ny))
...
def embed(tree):
    ...
    return {"W": W, "H": H, "zone": zone, "barrier": barrier,
            "chokepoints": chokepoints, "portals": portals, "pos": pos,
...
def ascii_preview(tree, em, cell=1):
    ...
    for y in range(0, H, cell):
        ...
        for x in range(0, W, cell):
            if (x, y) in choke:        row.append("+")
            elif (x, y) in portal:     row.append("O")
            elif (x, y) in seedpos:    row.append("@")
            elif barrier[y][x]:        row.append("#")
            else:                      row.append(glyph[zone[y][x] % len(glyph)])
...
if __name__ == "__main__":
    ...
    print(f"embed: {em['n_zones']} zones  chokepoints={len(em['chokepoints'])}  "
