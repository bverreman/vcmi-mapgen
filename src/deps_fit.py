...
PURPOSES = ["TOWN", "GUARD", "MINE", "DWELLING", "BANK", "REWARD_PICKUP", "RESOURCE_PILE",
            "TRANSPORT", "STAT_PERMANENT", "BONUS_TEMP", "SPELL_SKILL", "INFO",
            "MANA", "SPECIAL", "DECORATION"]
TERRAINS = list(range(10))


def _vec(purpose_cnt, terr_cnt, tiles, struct):
    return {"density": {p: 1000.0 * purpose_cnt.get(p, 0) / tiles for p in PURPOSES},
...
def feats_real(m):
    ...
    for lvl in m.terrain:
        for row in lvl:
            for t in row: terr[t.terrain] += 1
    pur = collections.Counter()
    for o in m.objects:
        r = vcmi_ids.resolve(o.obj_class, o.obj_subclass)
        if r: pur[R.TYPE2PURPOSE.get(r[0], "?")] += 1
    d = deps.extract_map(m)
    ng = max(1, d["n_gates"])
    struct = {"regions_per_1000": 1000.0 * d["n_reachable"] / tiles,
...
def feats_gen(fm, tree):
    ...
    struct = {"regions_per_1000": 1000.0 * tree["n_nodes"] / tiles,
...
def distance(a, b):
    ...
    st = abs(a["struct"]["regions_per_1000"] - b["struct"]["regions_per_1000"]) * 3 \
...
def points_gen(fm):
    return [(o["x"], o["y"], R.TYPE2PURPOSE.get(o["type"], "?")) for o in fm["objects"] if o["l"] == 0]
...
if __name__ == "__main__":
    ...
    print(f"knobs set from target: n_regions={params['n_target']} max_depth={params['max_depth']} "
          f"portal_frac={params['portal_frac']:.2f} biomes={ {k: round(v,2) for k,v in params['biome_weights'].items()} }")
    print(f"best seed={seed}   total={total:.1f}  (density={dist[1]:.1f} terrain={dist[2]:.1f} "
    ...
    shown = [d for d in sp_diffs if d[0][0] != d[0][1]][:8] + [d for d in sp_diffs if d[0][0] == d[0][1]][:5]
    ...
    combo = Image.new("RGB", (real_img.width + gen_img.width + 14, max(real_img.height, gen_img.height)), (255, 255, 255))
    combo.paste(real_img, (0, 0)); combo.paste(gen_img, (real_img.width + 14, 0))
