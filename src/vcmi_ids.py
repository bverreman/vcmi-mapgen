...
def _files(sub):
    out=[]
    for b in _BASES: out += glob.glob(f"{b}/**/config/{sub}/*.json", recursive=True) + glob.glob(f"{b}/{sub}/*.json")
    return sorted(set(out))

def _index_map(sub):           # identifier -> index  ==>  index -> identifier
    m={}
    for f in _files(sub):
        try: d=_relaxed(open(f).read())
        except Exception: continue
        for ident,obj in d.items():
            if isinstance(obj,dict) and isinstance(obj.get("index"),int): m.setdefault(obj["index"],ident)
    return m
...
def _load_objects():
    for f in _files("objects"):
        ...
        for tname,obj in d.items():
            if not isinstance(obj,dict) or not isinstance(obj.get("index"),int): continue
            subs={}
            for sname,s in (obj.get("types") or {}).items():
                if isinstance(s,dict) and isinstance(s.get("index"),int): subs[s["index"]]=sname
...
def resolve(obj_class, obj_subid):
    ...
    if tname in _BY_CREATURE: return tname, _CREATURE.get(obj_subid, "imp")
    if tname in _BY_FACTION:  return tname, _FACTION.get(obj_subid, "castle")
    if tname in _BY_HERO:     return tname, _HERO.get(obj_subid, "christian")
    if tname=="artifact":     return tname, _ARTIFACT.get(obj_subid, "spellBook")
    if tname=="spellScroll":  return tname, _SPELL.get(obj_subid, "magicArrow")
...
if __name__=="__main__":
    print(f"object classes: {len(_CLS2TYPE)}  creatures: {len(_CREATURE)}  factions: {len(_FACTION)}")
    for c,s in [(134,0),(101,0),(53,6),(79,6),(54,0),(98,0),(45,2),(5,0)]:
