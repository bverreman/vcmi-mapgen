...
import vmaplib, vmapwrite, render, generate3 as G3, generate4 as G4
...
def clearance_and_dist(W, H, towns, roads, RZONE=9, RROAD=2):
    ...
    for tx, ty in towns:
        stamp(tx, ty, RZONE)  # open zone around each town
    for rx, ry in roads:
        stamp(rx, ry, RROAD)  # clear corridor along roads
...
def place_wilderness(g, clear, dist, occ, counter, rnd, sweeps=2, RAMP=11, ALPHA=0.0, SCALE=2.0):
    ...
    objs = []
    field = G3.value_noise(
        W, H, max(6, W // 8), rnd
    )  # clumpy intensity: forest patches vs clearings
    ...
    def density(t, x, y):
        ...
        return min(
            0.22, SCALE * HD.get(t, 0) / 1000.0 * min(1.0, dist[y][x] / RAMP) * f
        )  # cap: never fully pack (no grid)

    for s in range(sweeps):
        order = [(x, y) for y in range(H) for x in range(W)]
        rnd.shuffle(order)
        for x, y in order:
            if (x, y) in occ or D[y][x] is not None or clear[y][x]:
                continue
            t = g[y][x]
            ...
            for tpl in pool:
                T = tpl["type"]
                lp = math.log(b0) + math.log(tpl["weight"] / totw)
...
if __name__ == "__main__":
    ...
    for i, (x, y) in enumerate(towns):
        ...
        for cc in G3.mask_blocked(to.get("template", {}), x, y):
            occ.add(cc)
    counter = [0]
    decor = place_wilderness(g, clear, dist, occ, counter, rnd)
    clearpct = sum(1 for y in range(H) for x in range(W) if clear[y][x]) * 100 // (W * H)
    print(
        f"towns={len(town_objs)}  decoration={len(decor)} ({1000 * len(decor) // (W * H)}/1000)  open/clear={clearpct}% of map"
    )
