...
import ontology as ON, vmaplib, vmapwrite
...
rmg_tpl = {}
fill_counts = collections.defaultdict(collections.Counter)
for p in glob.glob("/home/gabriel/.var/app/eu.vcmi.VCMI/data/vcmi/Maps/RandomMaps/*.vmap"):
    ...
    for g in [surf] + ([under] if under else []):
        for row in g:
            for c in row:
                if "_ri" not in c and "_ro" not in c:
                    fill_counts[c[:2]][c] += 1
    for o in objs:
    ...
    for lvl in m["terrain"]:
        for row in lvl:
            for cell in row:
                tiles[cell["t"]] += 1
    for o in m["objects"]:
        if ON.resolve(o["class"], o["subclass"])["purpose"] != "DECORATION":
            continue
        l = o["l"]
        if l >= len(m["terrain"]):
            continue
...
json.dump(
    {
        "fill": fill,
        "decor_by_terr": decor_by_terr,
        "human_decor_density": human_density,
    },
