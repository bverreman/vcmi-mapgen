...
sys.path.insert(0,"src"); import vmaplib, vmapwrite, render
...
def gen_terrain(w,h,palette,weights,rnd,nseeds):
    ...
    types=list(palette); wts=[weights[t] for t in types]
...
def mask_blocked(tpl, x, y):
    ...
    m=tpl.get("mask") or ["B"]
    H=len(m); W=max(len(r) for r in m)
    for r in range(H):
...
def scatter_decor(grid, level, occ, counter, scale=1.0):
    h=len(grid); w=len(grid[0]); objs=[]
    for y in range(h):
        for x in range(w):
            t=grid[y][x]; code=INT2CODE[t]
            ...
            counter[0]+=1
            objs.append({"instanceName":f"gen{counter[0]}","l":level,
                         "subtype":tpl.get("subtype","object"),"type":tpl.get("type"),
                         "template":tpl.get("template",{}),"x":x,"y":y})
    return objs

LAND={0:.12,1:.10,2:.42,3:.10,4:.08,5:.10,7:.05}     # surface biome weights
UNDER={6:.55,9:.30,2:.08,0:.07}                       # underground: subterranean+rock dominant
