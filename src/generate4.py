...
def cls(vtype):
    return VMAP2ENT.get(vtype, "DECORATION")


def _clamp(v):
    return min(max(v, 0.1), 12.0)
...
def _selfprof():
    ...
    for b in (0, 1):
        vals = [
            LIFT["lift"][e][e][b]
            for e in _DECO
            if LIFT["lift"].get(e, {}).get(e) and LIFT["lift"][e][e][b]
        ]
...
def place_lift_decor(g, level, occ, counter, rnd, sweeps=4, radius=6):
    ...
    for s in range(sweeps):
        ...
        for x, y in order:
            if len(objs) >= target:
                break
            if (x, y) in occ or D[y][x] is not None:
                continue
            t = g[y][x]
            pool = PAL["decor_by_terr"].get(str(t))
            if not pool:
                continue
            ...
            for dy in range(-radius, radius + 1):
                for dx in range(-radius, radius + 1):
                    ...
                    if 0 <= nx < W and 0 <= ny < H and D[ny][nx] is not None:
                        ...
                        b = 0 if cheb <= 2 else 1
                        contrib[(D[ny][nx], b)] += 1
            b0 = base(t)
            totw = sum(tp["weight"] for tp in pool)
            scores = []
            cand = []
            for tpl in pool:
                T = tpl["type"]
                lp = math.log(b0) + math.log(tpl["weight"] / totw)  # human-frequency weight
                ...
                cand.append(tpl)
            scores.append(0.0)
            cand.append(None)  # empty option (weight 1)
            mx = max(scores)
            ...
            objs.append(
                {
                    "instanceName": f"d{counter[0]}",
                    "l": level,
                    "subtype": chosen.get("subtype", "object"),
                    "type": chosen["type"],
...
if __name__ == "__main__":
    ...
    M4 = G3.MK.learn4(0)
    g = G3.macro_terrain(W, H, rnd)
    G3.markov_blend(g, M4, rnd, sweeps=2)
    towns = G3.place_towns(g, K=5, rnd=rnd, minsep=20)
    road = G3.carve_roads(g, towns, rnd)
    # town objects + occupancy
    base_town = None
    for p in glob.glob("/home/gabriel/.var/app/eu.vcmi.VCMI/data/vcmi/Maps/RandomMaps/*.vmap"):
        for o in vmaplib._relaxed(
            __import__("zipfile").ZipFile(p).read("objects.json").decode("utf-8", "replace")
        ):
            if o.get("type") == "town":
                base_town = o
                break
    ...
    for i, (x, y) in enumerate(towns):
        to = copy.deepcopy(base_town)
        to.update({"x": x, "y": y, "l": 0, "instanceName": f"town_{i}"})
        town_objs.append(to)
        for cc in G3.mask_blocked(to.get("template", {}), x, y):
            occ.add(cc)
    counter = [0]
    decor = place_lift_decor(g, 0, occ, counter, rnd, sweeps=6)
    print(
        f"towns={len(town_objs)}  lift-driven decoration={len(decor)}  ({1000 * len(decor) / (W * H):.0f}/1000 tiles)"
    )
    ...
    for T, pts in sorted(bytype.items(), key=lambda kv: -len(kv[1]))[:6]:
        ...
        dens = n / (W * H)
        exp = obs and (n * 24 * dens)  # 24 tiles in 5x5 ring
    ...
    for i, (pid, pl) in enumerate(list(header.get("players", {}).items())):
        pl["mainTown"] = (
            {"generateHero": True, "l": 0, "x": towns[i][0], "y": towns[i][1]}
            if i < len(towns)
            else None
        )
