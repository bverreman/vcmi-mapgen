...
PURPOSE_RGB = {
 "TOWN":(255,255,255),"MINE":(255,170,0),"RESOURCE_PILE":(255,215,0),
 "REWARD_PICKUP":(255,105,180),"GUARD":(220,20,20),"DWELLING":(160,32,240),
 "STAT_PERMANENT":(0,200,255),"BONUS_TEMP":(0,255,170),"MANA":(80,80,255),
 "SPELL_SKILL":(140,0,200),"INFO":(200,200,200),"BANK":(255,60,60),
 "QUEST_GATE":(255,140,0),"TRANSPORT":(0,255,0),"WATER_TRANSPORT":(0,255,255),
 "TERRAIN_MODIFIER":(180,180,0),"HERO":(255,0,255),"SPECIAL":(150,150,150),
}
...
def render_level(level_terrain, objects, w, h, title=""):
    ...
    for y,row in enumerate(level_terrain):
        for x,cell in enumerate(row):
            ...
            if isinstance(cell,dict) and cell.get("road"): r,g,b=min(r+40,255),min(g+30,255),b//2
            for dy in range(TILE):
                for dx in range(TILE):
                    px[x*TILE+dx, y*TILE+dy] = (r,g,b)
    d = ImageDraw.Draw(img)
    for o in objects:
        if o["l"] != level_terrain_level: continue
...
def render_map(mapjson, out_png):
    ...
    levels = mapjson["terrain"]; w=mapjson["width"]; h=mapjson["height"]
    imgs=[]
...
if __name__=="__main__":
    ...
    os.makedirs("/mnt/data/workspace/vcmi-mapgen/out/render", exist_ok=True)
    for key in sel:
        f=[p for p in glob.glob("/mnt/data/workspace/vcmi-mapgen/out/maps/*.json") if key.replace(".h3m","") in p]
        if not f: print("no match for",key); continue
        m=json.load(open(f[0]))
        outp=f"/mnt/data/workspace/vcmi-mapgen/out/render/{os.path.basename(f[0]).replace('.json','')}.png"
        render_map(m, outp)
        print(f"rendered {m['name']} ({m['width']}x{m['height']}, {len(m['terrain'])} lvl) -> {outp}")
