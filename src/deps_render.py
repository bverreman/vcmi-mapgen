...
TCOL = {0: (150, 110, 70), 1: (220, 205, 150), 2: (70, 150, 60), 3: (235, 240, 245),
        4: (90, 120, 80), 5: (170, 150, 110), 6: (120, 90, 130), 7: (70, 60, 60),
        8: (40, 90, 180), 9: (50, 45, 45)}
...
def occ_tiles(x, y, mask):
    ...
    for r, row in enumerate(mask):
        for c, ch in enumerate(row):
            if ch != 'V': yield (x - (w - 1 - c), y - (h - 1 - r))

def role(o):
    t = o["type"]
    if t == "town" or t == "randomTown": return (10, 10, 10)            # towns: black
    if "Monster" in t or t == "monster": return (220, 30, 30)          # guards: red
    if t.startswith("monolith") or t == "subterraneanGate": return (200, 60, 230)  # portals: magenta
    if t in ("mine", "windmill") or "Dwelling" in t or "Bank" in t or t == "creatureBank" \
       or t == "crypt": return (255, 180, 0)                            # high reward: gold
    if t in ("resource", "randomResource", "treasureChest", "campfire", "scholar") \
       or "Artifact" in t: return (255, 240, 120)                       # pickups: pale gold
    return None

def render(fm, out, scale=9):
    terr = fm["terrain"][0]; H = len(terr); W = len(terr[0])
    ...
    for y in range(H):
        for x in range(W):
            ...
            if c.get("ot"): col = (110, 80, 40)                        # road overlay: brown
    ...
    for o in fm["objects"]:
        if not is_mtn(o["type"]): continue
        for (tx, ty) in occ_tiles(o["x"], o["y"], o["mask"]):
            if 0 <= tx < W and 0 <= ty < H:
                d.rectangle([tx * scale, ty * scale, tx * scale + scale - 1, ty * scale + scale - 1],
                            fill=(95, 85, 80))
    for o in fm["objects"]:
        if is_mtn(o["type"]): continue
        col = role(o)
        if t := col:
