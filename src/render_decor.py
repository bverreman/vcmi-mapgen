...
import vmapwrite, vmaplib, render, generate4 as G4
...
def type_color(t):
    ...
    def has(*k):
        return any(w in s for w in k)

    if has("oak", "pine", "tree", "spruce", "willow", "yucca", "palm"):
        return (30, 120, 40)  # trees green
    if has("mountain"):
        return (115, 95, 80)  # mountain brown
    if has("rock", "outcrop", "stony", "canyon", "cliff"):
        return (120, 120, 120)  # rock grey
    if has("deadveg", "dead", "shrub", "bush", "stump", "log", "mound", "hedge", "moss"):
        return (120, 110, 65)  # scrub
    if has("cactus"):
        return (80, 150, 60)
    if has("flower", "mandrake"):
        return (230, 120, 200)
    if has("mushroom"):
        return (180, 90, 200)
    if has("reef", "kelp", "lake", "willowtree"):
        return (40, 160, 170)  # water decor teal
    if has("crater", "lava", "volcano", "fissure"):
        return (170, 60, 40)  # volcanic
    if has("sand", "dune"):
        return (210, 190, 120)
    return (90, 110, 70)
    ...
    for li, g in enumerate(grids):
        ...
        for y, row in enumerate(g):
            for x, c in enumerate(row):
                t = vmaplib.TERR.get(c[:2], 2)
                r, gg, b = render.TERRAIN_RGB.get(t, (0, 0, 0))
                for dy in range(TILE):
                    for dx in range(TILE):
                        px[x * TILE + dx, y * TILE + dy] = (r, gg, b)
        d = ImageDraw.Draw(img)
        for o in objs:
            if o.get("l", 0) != li:
                continue
            m = o.get("template", {}).get("mask", [])
            if not m or any("A" in rr for rr in m):
                continue  # decoration only
