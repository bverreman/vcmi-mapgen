...
VALUE = {
    98: 10,
    77: 10,  # town / random town
    53: 6,
    220: 6,  # mine / abandoned mine
    17: 7,
    18: 7,
    19: 7,
    20: 7,
    ...
    85: 6,
    108: 6,
    63: 6,  # banks: utopia/bank/crypt/derelict/shipwreck/tomb/pyramid
    36: 8,  # grail
    5: 5,
    64: 5,
    65: 5,
    ...
    67: 5,
    68: 5,
    69: 5,  # artifact + random tiers
    6: 5,
    93: 4,  # pandora, spell scroll
    104: 3,
    7: 3,
    113: 3,
    102: 3,
    88: 3,
    89: 3,
    90: 3,  # university/black market/witch/tree/shrines
    81: 3,
    39: 3,
    105: 3,
    41: 3,  # scholar/lean-to/wagon/marletto-type
    101: 2,
    12: 2,
    79: 2,
    76: 2,  # treasure chest/campfire/resource
    28: 1,
    82: 1,
    59: 1,
    86: 1,  # flotsam/sea chest/bottle/survivor
}
GATE_TYPE = {}
for c in MONSTER:
    GATE_TYPE[c] = "guard"
...
for c in GATE_KEY:
    GATE_TYPE[c] = "key"
for c in GATE_GARRISON:
    GATE_TYPE[c] = "garrison"
for c in GATE_QUEST:
    GATE_TYPE[c] = "quest"
...
def extract_map(m):
                ...
                while stack:
                    ...
                    for dx, dy in NB8:
                        ...
                        if (
                            0 <= nx < W
                            and 0 <= ny < H
                            and passable[l][ny][nx]
                            and not guarded[l][ny][nx]
                            and label[l][ny][nx] < 0
                        ):
                            label[l][ny][nx] = nid
                            stack.append((nx, ny))
                region_tiles.append(size)
                nid += 1
    NR = nid
    ...
    for rw in rewards:
        ...
        if rs:
            ...
            region_value[rid] += rw["value"]
            region_reward_n[rid] += 1
...
def aggregate(paths):
    ...
    for p in paths:
        try:
            ...
            print("FAIL", os.path.basename(p), repr(e)[:60])
            continue
    ...
    for e in edge_rows:
        g = bd[e["depth"]]
        g["val"].append(e["child_value"])
        g["n"] += 1
        if e["strength"]:
            g["str"].append(e["strength"])
    ...
    agg = {
        ...
        "branching_by_depth": {
            d: round(sum(v) / len(v), 2) for d, v in sorted(branch_by_depth.items()) if v
        },
        "branching_overall": pct([n for v in branch_by_depth.values() for n in v]),
        "commons_tiles": pct(commons_tiles),
        "pocket_tiles": pct(pocket_tiles),
        "gradient_by_depth": grad,
        "per_map": per_map,
    }
    out = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "out", "deps.json"
    )
