...
def obj_color(cls):
    if cls in (98, 77):
        return (10, 10, 10)  # town: black
    if cls in deps.MONSTER:
        return (225, 30, 30)  # guard: red
    if cls in deps.PORTAL:
        return (200, 60, 230)  # portal: magenta
    if cls in (53, 220):
        return (255, 180, 0)  # mine: gold
    if cls in (17, 18, 19, 20, 216, 217, 218):
        return (255, 140, 0)  # dwelling: orange
    if cls in (16, 25, 84, 24, 85, 108, 63):
        return (255, 110, 0)  # bank
    if cls in (5, 65, 66, 67, 68, 69, 93):
        return (0, 210, 210)  # artifact: cyan
    if cls in (79, 76, 101, 12, 82, 28):
        return (255, 240, 130)  # pickup: pale
    return None
...
def render_real(m, lvl, scale):
    ...
    for o in m.objects:  # blocking decoration (mountains/forests) -> gray
        if o.l != lvl or o.obj_class in GAMEPLAY:
            continue
    ...
    for y in range(H):
        for x in range(W):
            ...
            if rid < 0:
                ...
                col = depth_color(
                    dd["depth"][rid]
                )  # reachable region, coloured by progression depth
            else:
...
if __name__ == "__main__":
    ...
    combo.paste(real, (0, 0))
    combo.paste(struct, (real.width + gap, 0))
    ...
    combo.save(out)
    real.save(out.replace("showcase_", "real_"))
    struct.save(out.replace("showcase_", "struct_"))
