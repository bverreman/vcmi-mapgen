...
def test_protected_web_covers_gate_bands():
    ...
    for rep, band in _zone_gate_bands(ts1, zones, 1, open_frac=0.5):
...
def test_border_bias_densifies_front():
    ...
    from vcmi_mapgen.steps.vegetation import sample as PP
    from vcmi_mapgen.kit.geometry import edge_dist
    from vcmi_mapgen.kit.topology import plan_entrances, _zone_fronts
    ...
    def zone_pass(zid, ts, seed, border_bias=True):
        ...
        front = set().union(*_zone_fronts(ts, zones, zid).values())
